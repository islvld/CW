...
void Widget2::on_pushButton_8_clicked() { //Добавить
 QString text = ui->lineEdit_2->text().trimmed().simplified();
 ...
 if (list0.count()) { //Уже есть такой - заменить элемент
  ...
  on_pushButton_6_clicked(); //Удалить старый
...
void Widget2::on_pushButton_6_clicked() { //Удалить
 ...
 ui->lineEdit_2->setText(item->text()); //Скопировать текст удалённого
 ui->listWidget->setCurrentRow(row);
}

void Widget2::on_pushButton_7_clicked() { //Поиск
...
void Widget2::on_pushButton_9_clicked() { //Сортировка
 //ui->listWidget->setSortingEnabled(true);
 static int Order = Qt::DescendingOrder;
 if (Order==Qt::AscendingOrder) {
  Order = Qt::DescendingOrder;
  ui->listWidget->sortItems(Qt::DescendingOrder);
  ui->pushButton_9->setText(QString(tr("Sort(v)")));
 }
 else {
  Order = Qt::AscendingOrder;
  ui->listWidget->sortItems(Qt::AscendingOrder);
  ui->pushButton_9->setText(QString(tr("Sort(^)")));
 }
}

void Widget2::on_pushButton_4_clicked() { //Очистить
 QMessageBox msgBox;
...
void Widget2::on_listWidget_clicked(const QModelIndex &index){
 this->unselectAll();
 ui->listWidget->setSelectionMode(QAbstractItemView::SingleSelection);
 ui->lineEdit_2->setText( //Получить текущий элемент как строку
  ui->listWidget->currentItem()->text());
}

QString Widget2::itemCount()
{
